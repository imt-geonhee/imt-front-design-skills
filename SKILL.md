...
- **서브타이틀 / eyebrow 라벨**
  - 금지: 메인 타이틀 위에 붙는 작은 대문자 라벨(eyebrow, 예: "운영 콘솔")과 타이틀 아래 부연 설명 문단을 기본값으로 추가하는 것
  - 기준: 화면 목적이 타이틀 한 줄만으로 파악되면 eyebrow와 subtitle을 모두 생략한다
  - 예외: 한 화면에 여러 섹션이 있어 구분이 필요한 경우에만 eyebrow를 허용하고, 처음 진입한 사용자가 다음 행동을 알 수 없는 경우에만 subtitle 한 줄을 허용한다 — 두 조건 모두 아니면 타이틀과 입력 요소만으로 화면을 구성한다
...
- **핵심 카피 (제품/브랜드 문구)**
  - 원칙: 로그인 화면 태그라인, 제품 소개 문구, 브랜드 슬로건 등 제품·브랜드를 대표하는 핵심 문구는 AI가 임의로 확정하지 않는다
  - 절차: 초안 문구는 작성해서 제시하되, 결과물을 전달할 때 "이 문구는 사용자 확인이 필요합니다"라고 별도로 안내하고 확정 여부를 확인받는다
  - 적용 범위: 로그인 화면 좌측 패널 문구, 제품명 부제, 온보딩 첫 화면 문구 등 여러 화면에 반복 노출되는 핵심 카피에 적용한다
...
- eyebrow 라벨이나 subtitle 문단을 근거 없이 추가하지 않았는가
- 로그인 화면 태그라인 등 핵심 카피를 임의로 확정하지 않고 사용자 확인을 안내했는가
